...
class AdaIN_net(nn.Module):
    ...
    def forward(self, content, style, alpha=1.0):
        ...
        if self.training:  # training
            ...
            content_feat = self.encode(content)[-1]  # Get the last feature

            # Applying AdaIN
            t = self.adain(content_feat, style_feats[-1])
            t = alpha * t + (1 - alpha) * content_feat

            # decoding to get the transformed image
            g_t = self.decoder(t)
            ...
            #   your code here ...
            content_feat = self.encode(content)[-1]  # Get the last feature
            style_feat = self.encode(style)[-1]  # Get the last feature for style

            # Applying AdaIN
            t = self.adain(content_feat, style_feat)
            t = alpha * t + (1 - alpha) * content_feat

            # decoding to get the stylized image
